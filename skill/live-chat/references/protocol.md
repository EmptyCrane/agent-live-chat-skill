...
旧版 `state.json` 没有participants时，从消息和typing按首次出现顺序推导；没有session时补为idle。旧角色缺少行为或模型字段时，补为空行为设置、`requested=default`和空effective；旧session缺少model_policy时补为`inherit/ask`默认策略。保持现有schema和协议版本，不改写旧来源文件。
